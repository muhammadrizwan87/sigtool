...
class ColonLowerCase:
    ...
    def convert_to_lowercase(self, string: str) -> str:
        return string.lower()

    def process_signature_hashes(self, hashes: dict) -> dict:
        processed_hashes = {}
        for hash_type, hex_value in hashes.items():
            upper_hex_value = self.convert_to_lowercase(hex_value)
    ...
    def process_crc32_and_hashcode(self, results: dict) -> dict:
        ...
        for key, value in results.items():
            if isinstance(value, str):
                processed_results[key] = self.convert_to_lowercase(value)
            else:
